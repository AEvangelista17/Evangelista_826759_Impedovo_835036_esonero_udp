...
#include <winsock.h>
#include <ws2tcpip.h>
// CORREZIONE WARNING: La macro NO_ERROR viene definita qui per Windows
#define NO_ERROR 0
#else
#include <string.h>
#include <unistd.h>
...
int deserialize_response(const char *buffer, size_t len, weather_response_t *res) {
    ...
    // Deserializza status (Network Byte Order)
    uint32_t net_status;
...
int parse_and_validate_request(const char *request_str, char *type, char *city) {
    ...
    // Verifica che il primo token sia ESATTAMENTE un singolo carattere
    if (first_space - request_str != 1) {
        fprintf(stderr, "Errore: il tipo di richiesta ('%c') deve essere un singolo carattere.\n", request_str[0]);
        return 0;
    }

    *type = request_str[0]; // Estrai il type

    // La città inizia dopo il primo spazio (salta gli spazi iniziali)
    const char *city_start = first_space;
    while (*city_start == ' ') {
        city_start++;
    }

    // Verifica lunghezza città (max 63 caratteri)
    ...
    // Copia e assicura terminazione null
